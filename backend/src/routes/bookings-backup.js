...
export const bookingRoutes = new Elysia({ prefix: '/bookings' })
  ...
  .post('/', async ({ body, headers, set }) => {
    try {
      ...
      try {
        ...
        // Use validated data for booking
        const bookingData = {
          hotelId,
          roomTypeId,
          checkInDate,
      ...
      // Validate dates (final check)
      const checkIn = new Date(checkInDate);
      const checkOut = new Date(checkOutDate);
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      
      console.log('Final date validation:', {
        checkIn: checkIn.toISOString(),
        checkOut: checkOut.toISOString(),
        today: today.toISOString()
      });
      ...
      if (checkIn < today) {
        console.log('Final check: Check-in date is in the past');
        set.status = 400;
        return { error: 'Check-in date cannot be in the past' };
      }
      
      if (checkOut <= checkIn) {
        console.log('Final check: Check-out date is not after check-in date');
        set.status = 400;
        return { error: 'Check-out date must be after check-in date' };
      }
  ...
  // Alternative delete endpoint using POST (Admin only)
  .post('/:id/delete', async ({ params, headers, set }) => {
    try {
      ...
      // Delete related guest info first (if exists)
      const guestInfoDeleted = await sql`DELETE FROM guest_info WHERE booking_id = ${bookingId}`;
      console.log('Guest info deletion result:', guestInfoDeleted);
  ...
  // DELETE endpoints - placed at the end to avoid conflicts
  ...
  // Clear all bookings (Admin only - for testing)
  .delete('/admin/clear-all', async ({ headers, set }) => {
    try {
      // Authenticate admin
      const user = await authMiddleware({ headers, set });
  ...
  // Get all bookings for debugging (Admin only)
  .get('/admin/debug/all', async ({ headers, set }) => {
    try {
      // Authenticate admin
      const user = await authMiddleware({ headers, set });
